    ...
    def run(self, *arg):
        ...
        *arg :
        ...
        if self.model == "py":
            ...
            self.print_log(
                type="F", msg="Requested model currently not supported"
            )
...
if __name__ == "__main__":
    ...
    from register_template.controller import (
        controller as register_template_controller,
    )
    import pdb
    ...
    indata = np.random.randint(
        -(2**15) - 1, high=2**15, size=length
    ).reshape(-1, 1) + 1j * np.random.randint(
        -(2**15) - 1, high=2**15, size=length
    ).reshape(
        -1, 1
    )
